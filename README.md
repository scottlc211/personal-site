...
| `room` | 3D 房间装饰:`posterImage` 填图片路径即可替换墙上海报(建议竖幅 3:4,放 `assets/img/`),留空显示内置手绘海报,加载失败自动回退;`music` 配置背景音乐(见下方「背景音乐」) |
...
五斗柜上有一台可交互的唱片机:房间视角下点击它即可播放/暂停背景音乐。声音走 Three.js 空间音频(`PositionalAudio`),从唱片机的方向传来,随相机远近自然变化;播放时唱片会旋转、飘出音符,进入屏幕模式后音量自动减弱。

- 音源在 `assets/js/data.js` 的 `room.music` 配置:`src` 指向音频文件,`title` 是播放提示显示的曲名
- 音频文件放 `assets/media/`,**推荐 mp3(128kbps,一首歌约 3-5MB)**;浏览器要把文件整个下载并解码后才能播放,过大的文件会让点击后等很久
- 无损格式(flac/wav,动辄几十 MB)请先转码再部署,例如:`ffmpeg -i input.flac -b:a 128k bgm.mp3`;原始无损文件不必上传到服务器
- 受浏览器自动播放策略限制,音乐必须由用户点击触发,不会自动响起
- 使用有版权的商业歌曲存在侵权风险,公开部署建议换成免版权音乐(Pixabay Music、FreePD、incompetech 等)
...
- **唱片机**:点击五斗柜上的唱片机播放/暂停背景音乐(空间音频,详见「背景音乐」)
